...
function generateFileName(file, type) {
  const trimType = type.trim().replace(' ', '');
  
  return fileExists(file)
    ? `${trimType}-${file}-1.json`
    : `${trimType}-${file}.json`;
}
    ...
    try {
      ...
      for (const dir of directories) {
        ...
        for (const file of files) {
          ...
          const fileName = generateFileName(file, plugin.type);
          ...
          if (isDirectory(filePath)) {
            try {
              ...
              // If the model has a category, use it (the first one), otherwise default to the plugin type
              const category = model.categories ? model.categories[0] : plugin.type;
          
              const validModel = castSchema({ file: fileName, type: category, ...model });
